                                ...
                                <Reveal key={step.number} delay={0.2} direction={isEven ? 'right' : 'left'}>
                                    <div className={`flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-16 ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                                        ...
                                        <div className={`flex-1 w-full md:w-1/2 ${isEven ? 'md:text-right' : 'md:text-left'} pl-16 md:pl-0`}>
                                            <div className="group relative overflow-hidden rounded-[2rem] border border-white/[0.06] bg-black/40 p-8 md:p-10 backdrop-blur-xl transition-all duration-500 hover:border-[#F4751E]/30 hover:bg-black/60 shadow-[0_15px_40px_rgba(0,0,0,0.4)]">
                                                <div className="absolute inset-0 bg-gradient-to-br from-[#F4751E]/[0.03] to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                                                <div className="absolute -inset-x-32 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 transition-all duration-700 group-hover:opacity-100 group-hover:translate-x-1/2" />
                                                
                                                <div className={`flex flex-col gap-4 ${isEven ? 'md:items-end' : 'md:items-start'}`}>
                                                    <span className="text-5xl md:text-6xl font-extrabold text-white/[0.08] leading-none group-hover:text-[#F4751E]/[0.15] transition-colors duration-500">{step.number}</span>
                                                    <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{step.title}</h3>
                                                    <p className="text-base text-white/55 leading-relaxed font-light">{step.description}</p>
                                                </div>
                                            </div>
                                        </div>

                                        {/* Timeline Orb */}
                                        <div className="absolute left-[28px] md:left-1/2 md:relative md:left-0 -translate-x-1/2 md:translate-x-0 w-14 h-14 md:w-16 md:h-16 rounded-full border border-white/[0.1] bg-black/80 backdrop-blur-md flex items-center justify-center flex-shrink-0 z-20 group transition-all duration-500 hover:border-[#F4751E]/60 hover:shadow-[0_0_35px_rgba(244,117,30,0.4)] hover:scale-110 mt-2 md:mt-0 cursor-default">
